...
def generateATAC_new(Matrix, coords, chromname, seq_embedding, file_dir, epi_type, resou,  width=11, lower=1, positive=True, stop=5000):
    """
    ...
    for c in coords:
        ...
        if y-x < lower:
            ...
            try:
                window = Matrix[x-width:x+width,
                                y-width:y+width].toarray()
                ...
                if np.count_nonzero(window) < window.size*.1:
                    ...
                    if positive and p2LL < 0.1:
                        ...
                        if np.all(np.isfinite(window)):
                            seq_x = seq_embedding[(x - width) * resou : (x + width) * resou]
                            seq_y = seq_embedding[(y - width) * resou : (y + width) * resou]
                            seq_x = seq_x.reshape((2*width, resou, 13))
                            seq_x = seq_x.mean(axis=1)
                            seq_y = seq_y.reshape((2 * width, resou, 13))
                            seq_y = seq_y.mean(axis=1)
                            seq = (seq_x, seq_y)
                            # print('fin')
                            ...
                            if (chromname, x) not in epi_dict:
                                # (6, 50)
                                window_x = np.zeros((len(epi_type), 2 * width))
                            ...
                            if (chromname, y) not in epi_dict:
                                # (6, 50)
                                window_y = np.zeros((len(epi_type), 2 * width))
                                for i, epi in enumerate(epi_type):
                            ...
                            if negcount >= stop:
                                raise StopIteration
                            yield (window, node_features, tuple_edge, seq)
            except:
                pass


def generateATAC_woseq(Matrix, coords, chromname, file_dir, epi_type, resou,  width=11, lower=0, positive=True, stop=5000):
    """
    Generate training set
    :param coords: List of tuples containing coord bins
    :param width: Distance added to center. width=5 makes 11x11 windows
                ...
                if np.count_nonzero(window) < window.size*.1:
                    ...
                    if positive and p2LL < 0.1:
                        ...
                        if np.all(np.isfinite(window)):
                            ...
                            if negcount >= stop:
                                raise StopIteration
                            yield (window, node_features, tuple_edge)
            except:
                pass


def generateHiCFoundation(Matrix, coords, chromname, resou,  width=11, lower=0, positive=True, stop=5000):
    """
    Generate training set
    :param coords: List of tuples containing coord bins
    :param width: Distance added to center. width=5 makes 11x11 windows
