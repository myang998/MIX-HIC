...
class Downsampling1DTransformer(nn.Module):
    def __init__(self, embed_dim=64, dropout=0.0, num_heads=4, depth=2, seq_len=100):
        ...
        self.layer_norm1 = nn.LayerNorm(embed_dim * 2)
        ...
        self.layer_norm2 = nn.LayerNorm(embed_dim * 4)
...
class Downsampling2DTransformer(nn.Module):
    def __init__(self, num_heads=4, patch_size=2, embed_dim=64, in_dim=2, dropout=0.0,
                 ...
                 token_projection='linear', dowsample=Downsample, mlp_ratio=2,use_repo=False):
...
class UformerGraphFuse(nn.Module):
    def __init__(self, num_heads=4, patch_size=2, embed_dim=64, in_dim=2, dropout=0.0,
                 depths=2, token_mlp='leff', mlp_ratio=2, mode='pretrain', loss_mode='translation'):
        super().__init__()
        """
        ...
        print(f'num depths: {depths}, embed_dim: {embed_dim}, mode: {mode}, loss_mode: {loss_mode}')
        ...
        self.seq_downsampleTrans = Downsampling1DTransformer(embed_dim=embed_dim, dropout=dropout, num_heads=num_heads, depth=depths)
    ...
    def contrastive_loss2(self, epi, map):
        # epi = F.normalize(epi, dim=-1)
        # map = F.normalize(map, dim=-1)
        logits = epi @ map.transpose(-2, -1)

        # Positive keys are the entries on the diagonal
        labels = torch.arange(len(epi), device=epi.device)
        loss = F.cross_entropy(logits / self.temp, labels, reduction='mean')

        # loss = torch.mean(torch.norm(epi - map / torch.norm(epi, dim=1, keepdim=True), dim=1))

        return loss

    def margin_ranking(self, epi, map):
        logits = epi @ map.transpose(-2, -1)
        logits = logits / self.temp

        # Positive samples: diagonal elements
        positive_samples = torch.diag(logits)

        # Negative samples: randomly select non-diagonal elements
        negative_samples = []
        n = logits.size(0)
        for i in range(n):
            mask = torch.ones(n, dtype=bool, device=logits.device)
            mask[i] = False  # Mask out the positive sample
            negative_samples.append(logits[i, mask].mean())  # Take mean of negatives for stability
        negative_samples = torch.stack(negative_samples)

        # Target tensor indicating that positive should be higher than negative
        target = torch.ones_like(positive_samples)

        # Calculate margin ranking loss
        loss = self.margin_loss(positive_samples, negative_samples, target)
        return loss
    ...
    def all_loss(self, epi, map):
        ...
        # contra_loss = self.margin_ranking(epi_share_mean, map_share_mean)
        ...
        return (contra_loss, trans_loss, orth_loss), (epi_concat_embed, map_concat_embed)

    def forward(self, map_data, graph_data):
        """
        ...
        # 1.sequence BERT
        x2 = map_data
        x2 = self.to_patches(x2)
        x2 = self.patch_embedding(x2)

        map_embed = self.map_downsampleTrans(x2)

        all_loss, (epi_concat_embed, map_concat_embed) = self.all_loss(epi_embed, map_embed)
