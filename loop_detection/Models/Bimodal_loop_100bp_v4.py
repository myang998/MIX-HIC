...
class UformerGraphFuse(nn.Module):
    def __init__(self, img_size=50, num_heads=4, patch_size=2, embed_dim=64, in_dim=2, dropout=0.0,
                 depths=2, qkv_bias=True, qk_scale=None, norm_layer=nn.LayerNorm, token_mlp='leff',
                 token_projection='linear', mlp_ratio=2, device='cuda:0', modality='bimodal', mode='finetune', loss_mode='translation', use_repo=False):
        super().__init__()
        """
        ...
        self.map_downsampleTrans = Downsampling2DTransformer(num_heads=num_heads, patch_size=patch_size, embed_dim=embed_dim, in_dim=in_dim, dropout=dropout, depths=depths, token_mlp=token_mlp,
                                                             mlp_ratio=mlp_ratio, use_repo=use_repo)

        if self.loss_mode == 'contras' or self.loss_mode == 'geometric':
            classifier_dim = embed_dim * 2
        else:
            classifier_dim = embed_dim * 8
        ...
        self.fusion_block = BasicTransformerLayer(dim=embed_dim * 8,
                                                  ...
                                                  norm_layer=norm_layer,
                                                  use_checkpoint=False,
                                                  token_projection=token_projection, token_mlp='mlp',use_rope=use_repo)
    ...
    def forward(self, graph_data, map_data=None):
        """
        ...
        if self.modality == 'infer_map':
            ...
            # fusion_embed = epi_concat_embed
        elif self.modality == 'map':
            fusion_embed = map_embed
        ...
        output = torch.mean(fusion_embed, dim=1)
