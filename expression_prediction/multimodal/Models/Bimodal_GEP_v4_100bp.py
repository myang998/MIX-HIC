...
from multimodal.Models.uformer_utils import *
...
class Pooling1DEncoder(nn.Module):
    def __init__(self, embed_dim=64, dropout=0.0, num_layer=4):
        ...
        #         nn.Sequential(
        #             nn.Dropout(dropout),
        #             nn.Conv1d(embed_dim, embed_dim, kernel_size=3, padding='same', dilation=2 ** i),
...
class Downsampling2DTransformer(nn.Module):
    def __init__(self, img_size=50, num_heads=4, patch_size=2, embed_dim=64, in_dim=2, dropout=0.0,
                 ...
                 token_projection='linear', dowsample=Downsample, mlp_ratio=2):
...
class UformerGraphFuse(nn.Module):
    def __init__(self, img_size=50, num_heads=4, patch_size=2, embed_dim=64, in_dim=2, dropout=0.0,
                 depths=2, qkv_bias=True, qk_scale=None, norm_layer=nn.LayerNorm, token_mlp='leff',
                 token_projection='linear', dowsample=Downsample, mlp_ratio=2, device='cuda:0', modality='bimodal', mode='finetune', loss_mode='translation', use_rope=False):
        super().__init__()
        """
            loss_mode: contras, orth, gauss, all, geometric, translation
            mode: pretrain, finetune
            modality: bimodal, map, epi, infer_map
        """
        self.mode = mode
        ...
        self.map_downsampleTrans = Downsampling2DTransformer(img_size=img_size, num_heads=num_heads, patch_size=patch_size, embed_dim=embed_dim, in_dim=in_dim, dropout=dropout, depths=depths, token_mlp=token_mlp,
                                                             mlp_ratio=mlp_ratio)
    ...
    def translation_loss_seq(self, epi_concat, map_concat):
        # print(epi_concat.shape)
        # print(map_concat.shape)
    ...
    def forward(self, map_data, graph_data):
        ...
        output = self.seq_upsampleTrans(fusion_embed, encode0, encode1, encode2)

        output = self.classifier(output)
        output = output.view(output.shape[0], -1)
        return output, all_loss
